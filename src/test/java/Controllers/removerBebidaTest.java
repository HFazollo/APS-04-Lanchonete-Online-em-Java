...
public class removerBebidaTest {
    ...
    @Test
    void testRemoverBebidaComSucesso() throws Exception {
        String jsonBebida = "{\"id\":1,\"nome\":\"Cerveja\",\"descricao\":\"Lata 350ml\",\"quantidade\":10,\"ValorCompra\":2.50,\"ValorVenda\":4.00,\"tipo\":\"Alcoólica\"}";
        ...
        try (MockedConstruction<ValidadorCookie> mockedValidator = Mockito.mockConstruction(ValidadorCookie.class,
                (mock, context) -> {
                    when(mock.validarFuncionario(any(Cookie[].class))).thenReturn(true);
                });
                MockedConstruction<DaoBebida> mockedDao = Mockito.mockConstruction(DaoBebida.class)) {

            new removerBebida().doPost(request, response);
        }

        writer.flush();
        assertEquals("Bebida Removida!", stringWriter.toString().trim());
    }

    @Test
    void testRemoverBebidaSemAutorizacao() throws Exception {
        mockRequestInputStream("{}");

        Cookie[] cookies = { new Cookie("tipoUsuario", "cliente") };
        when(request.getCookies()).thenReturn(cookies);
        ...
        try (MockedConstruction<ValidadorCookie> mockedValidator = Mockito.mockConstruction(ValidadorCookie.class,
                (mock, context) -> {
                    when(mock.validarFuncionario(any(Cookie[].class))).thenReturn(false);
                })) {
